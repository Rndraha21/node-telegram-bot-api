async function sendNews(bot, chatId) {
  ...
  try {
    await bot.sendMessage(chatId, "⌛ Tunggu sebentar sedang mencari berita...");
    const response = await fetch(newsApi);
    const data = await response.json();

    await new Promise((r) =>
      setTimeout(() => {
        bot.sendMessage(chatId, "🎉 Ditemukan 5 berita terbaru untuk mu...");
        r();
      }, 3000)
    );
    ...
    console.error(err);
    return bot.sendMessage(chatId, "⚠️ Ups... sory saya gagal mendapatkan berita.");
  }
}
